...
BOOL PegRect::Contains(PegPoint Test) const {
  if (Test.x >= wLeft && Test.x <= wRight && Test.y >= wTop &&
      Test.y <= wBottom) {
    return TRUE;
  }
  return FALSE;
...
BOOL PegRect::Contains(SIGNED Testx, SIGNED Testy) const {
  if (Testx >= wLeft && Testx <= wRight && Testy >= wTop && Testy <= wBottom) {
    return TRUE;
  }
  return FALSE;
}
...
BOOL PegRect::Contains(const PegRect &That) const {
  if (That.wLeft >= wLeft && That.wRight <= wRight && That.wTop >= wTop &&
...
BOOL PegRect::Overlap(const PegRect &That) const {
  if (That.wLeft <= wRight && That.wTop <= wBottom && That.wBottom >= wTop &&
      That.wRight >= wLeft) {
    return TRUE;
  }
  return FALSE;
...
/*--------------------------------------------------------------------------*/
PegRect PegRect::operator^=(const PegRect &Rect) {
  if (wRight < Rect.wLeft || wLeft > Rect.wRight || wTop > Rect.wBottom ||
      wBottom < Rect.wTop) {
  ...
  if (wLeft >= Rect.wLeft && wRight <= Rect.wRight) {
    if (wTop < Rect.wTop) {
...
BOOL PegRect::operator!=(const PegRect &Rect) const {
  if (Rect.wTop != wTop || Rect.wBottom != wBottom || Rect.wLeft != wLeft ||
      Rect.wRight != wRight) {
    return TRUE;
  }
  return FALSE;
